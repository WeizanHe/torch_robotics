...
from torch_robotics.torch_kinematics_tree.models.robots import Differentiable2LinkPlanar, DifferentiableDenso
...
if __name__ == "__main__":
    ...
    print("\n===========================Denso Model===============================")
    diff_denso = DifferentiableDenso(gripper=False, device=device)
    diff_denso.print_link_names()
    print(f'joint limits: {diff_denso.get_joint_limits()}')
    print(f'n dofs: {diff_denso._n_dofs}')

    print()
    # Compute forward kinematics all task space links
    with TimerCUDA() as t:
        q = torch.rand(batch_size, diff_denso._n_dofs).to(device).requires_grad_(True)
        data_links_fk = diff_denso.compute_forward_kinematics_all_links(q)
    ...
    with TimerCUDA() as t:
        link_analytical_jac = diff_denso.compute_analytical_jacobian_all_links(q)
    ...
    # with Timer() as t:
    #     ee_pos, ee_rot, lin_jac, ang_jac = diff_denso.compute_forward_kinematics_and_geometric_jacobian(q, torch.zeros_like(q), 'ee_link')
