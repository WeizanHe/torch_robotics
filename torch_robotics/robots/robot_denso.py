...
from torch_robotics.torch_kinematics_tree.models.robots import DifferentiableDenso
...
class RobotDenso(RobotBase):
    ...
    def __init__(self,
        ...
        self.link_name_grasped_object = None

        self.diff_denso = DifferentiableDenso(
            gripper=self.gripper, device=tensor_args['device'], grasped_object=grasped_object,
            use_collision_spheres=use_collision_spheres
        )

        self.jl_lower, self.jl_upper, _, _ = self.diff_denso.get_joint_limit_array()
        ...
        #         idx = self.diff_denso._name_to_idx_map[j]
        ...
        if use_collision_spheres:
            link_names_for_object_collision_checking = self.diff_denso.link_collision_names
            link_margins_for_object_collision_checking = to_torch(self.diff_denso.link_collision_margins, **tensor_args).view(-1, 1)
        ...
        for link_name in link_names_for_object_collision_checking:
            idx = self.diff_denso._name_to_idx_map[link_name]
            link_idxs_for_object_collision_checking.append(idx)
        ...
        # link_names_for_self_collision_checking = self.diff_denso.get_link_names()
    ...
    def fk_map_collision_impl(self, q, **kwargs):
        ...
        link_pose_dict = self.diff_denso.compute_forward_kinematics_all_links(q, return_dict=True)
        # link_tensor = convert_link_dict_to_tensor(link_pose_dict, self.link_names_for_object_collision_checking)
        link_all_tensor = convert_link_dict_to_tensor(link_pose_dict, self.diff_denso.get_link_names())
    ...
    def get_EE_pose(self, q):
        return self.diff_denso.compute_forward_kinematics_all_links(q, link_list=[self.link_name_ee])
    ...
    def render(self, ax, q=None, color='blue', arrow_length=0.15, arrow_alpha=1.0, arrow_linewidth=2.0,
        ...
        # draw skeleton
        skeleton = get_skeleton_from_model(self.diff_denso, q, self.diff_denso.get_link_names())
        skeleton.draw_skeleton(ax=ax, color=color)

        # forward kinematics
        fks_dict = self.diff_denso.compute_forward_kinematics_all_links(q.unsqueeze(0), return_dict=True)
